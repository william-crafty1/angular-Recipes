...
function AppService($location, $http) {
    var self = this;
    var key = "305f31f55a51a74e251a32a89c7ac20c";
    var appID = "fe34feed";
    var healthSearch = ["vegan", "peanut-free"]; // array for the health filter option
    var dietSearch = ["high-protein", "low-carb", "low-fat"]; //array for the diet filter options
    ...
    self.Search = function (input, id, idx) {
        ...
        $location.path("/recipeList"); //may change path name later for html
    ...
    self.addFave = function (item) {
        for (var i = 0; i < self.faveArray.length; i++) {
